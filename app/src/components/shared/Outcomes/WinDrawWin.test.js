...
it('renders without crashing', () => {
    shallow(<WinDrawWin outcomes={List()} />);
});
...
it('renders outcomes', () => {
    ...
    const wrapper = shallow(<WinDrawWin outcomes={outcomes} />);

    expect(wrapper.find(Outcome)).toHaveLength(4);
    expect(wrapper).toContainReact(<Outcome outcome={outcome1} title="Win" />);
    expect(wrapper).toContainReact(<Outcome outcome={outcome2} title="Win" />);
    expect(wrapper).toContainReact(<Outcome outcome={outcome3} title="Win" />);
    expect(wrapper).toContainReact(<Outcome outcome={outcome4} title="Draw" />);
});
