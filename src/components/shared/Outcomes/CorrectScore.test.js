...
it('renders without crashing', () => {
    shallow(<CorrectScore outcomes={List()} />);
});
...
it('renders outcomes', () => {
    ...
    const wrapper = shallow(<CorrectScore outcomes={outcomes} />);
    ...
    expect(homeColumn).toContainReact(<Outcome outcome={outcome1} title="1-0" />);
    expect(awayColumn).toContainReact(<Outcome outcome={outcome2} title="1-2" />);
    expect(awayColumn).toContainReact(<Outcome outcome={outcome3} title="3-4" />);
    expect(drawColumn).toContainReact(<Outcome outcome={outcome4} title="2-2" />);
});
